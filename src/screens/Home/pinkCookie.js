...
function PinkCookie({width, height, midOffset}) {
    ...
    const cookieDiameter = width / 3.1;
    ...
    if (Math.abs(percentageX) > threshold) {
        offsetX = -(percentageX > 0 ? (percentageX - threshold): (percentageX + threshold)) * 15;
    }
    if (Math.abs(percentageY) > threshold) {
        offsetY = -(percentageY > 0 ? (percentageY - threshold): (percentageY + threshold)) * 15;
    }
    useEffect(() => {
